...
# 尝试导入联邦学习优化器
try:
    from app.services.federated_model_optimizer import federated_model_optimizer
    FEDERATED_OPTIMIZATION_AVAILABLE = True
except ImportError:
    FEDERATED_OPTIMIZATION_AVAILABLE = False
    logger.warning("联邦学习优化器不可用")
...
class ModelSelector:
    ...
    def select_model(
        ...
        # 如果启用联邦学习优化，尝试应用优化
        if use_federated_optimization and FEDERATED_OPTIMIZATION_AVAILABLE:
            try:
