...
| StepExecutor | 执行 Step、记录输入输出和耗时 | 必须自研 | 每一步都要保存输入、输出、错误、风险和审计 | 保留框架无关的 Adapter 扩展点 |
...
| LangGraph | 适合复杂 Agent 状态图和 Python 生态工具调用 | 和 Java 治理栈割裂，状态、审计、权限落库需要二次封装 | 作为竞品与技术参照，不纳入当前实现路线 |
...
第一阶段建议自研轻量 Workflow Runtime。现有 Core Native ACG 已证明此路线可行，但正式版本应将状态、Trace、Review、Checkpoint 落到 PostgreSQL。外部 Agent 编排框架不纳入当前实现路线；若未来出现自研 ACG 无法覆盖的明确需求，再通过框架无关 Adapter 做专项评估。Temporal 可作为后期生产级长周期可靠任务执行引擎。不建议第一阶段引入重型 BPMN 工作流。
...
| 外部 Agent 编排框架与 Java 技术栈割裂 | 状态、审计、权限分散 | 当前以 Core Native ACG 收敛执行路径，外部框架仅作技术参照 |
...
LangGraph 不应作为核心内核，也不纳入当前阶段的 Adapter 实现计划。Temporal 同样不应第一阶段直接引入，可在长周期任务、可靠重试、生产级调度需求明确后再评估。
...
| LangGraph | 复杂 Agent 状态图、Python 工具生态强 | 竞品与技术参照 | 与 Java 治理栈割裂，需要二次封装 |
...
> 我们不是没有比较 LangGraph、Temporal、Dify、Qdrant、Keycloak、Kubernetes 等成熟方案，而是根据第一阶段目标选择了最小可验证路线。MVP 的核心不是堆组件，而是验证 AgentOS 的治理闭环：Task、Workflow、Step、Evidence、Review、Trace、Checkpoint、Report。外部框架只在出现明确且自研能力无法覆盖的需求时重新评估，避免第一阶段技术过载。
