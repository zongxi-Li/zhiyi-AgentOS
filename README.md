...
知弈是一个面向信创与信息敏感行业的职业智能体操作系统。系统采用 **前端(Vue 3) + 后端(Spring Boot) + AI服务(FastAPI)** 三层架构，支持律师、教师、程序员、作家四种专业Agent角色，具备 ReAct 自主规划执行、技能调用、知识检索、流式对话等核心能力。AI 引擎采用 DeepSeek + 通义千问双引擎，文本生成由 DeepSeek 主力驱动。
